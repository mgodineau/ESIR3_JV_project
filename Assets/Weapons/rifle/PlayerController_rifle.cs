...
public class PlayerController_rifle: MonoBehaviour {
	...
	private void Update() {
        ...
        {
			if (!_anim.GetCurrentAnimatorStateInfo(0).IsName("run")) _anim.SetTrigger(Run);
			//print("run");
		}
		else
        {
			if (!_anim.GetCurrentAnimatorStateInfo(0).IsName("idle")) _anim.SetTrigger(Stop);
				//print("idle");	
		}
